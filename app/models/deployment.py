...
class DeploymentRequest(db.Model):
    ...
    # Relationships
    worker = db.relationship('Worker', backref='deploy_requests', lazy=True)
    company = db.relationship('Company', backref='deploy_requests', lazy=True)
    submitter = db.relationship('User', foreign_keys=[submitted_by], backref='submitted_requests', lazy=True)
    reviewer = db.relationship('User', foreign_keys=[reviewed_by], backref='reviewed_requests', lazy=True)
